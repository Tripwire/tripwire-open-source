...
#define TSS_PARSER_EXCEPTION( except ) \
    TSS_BEGIN_EXCEPTION_NO_CTOR( except, eParserHelper ) \
        except( const TSTRING& strMsg, int nLine = CURRENT_LINE ) \
            :   eParserHelper( strMsg, nLine ) \
                {}; \
    TSS_END_EXCEPTION()
...
public:
    ...
    static AcceptState  GetLastState() { return UnderneathTopState(); };
    static void         ToggleTopState();
    static AcceptState  GetOppositeState( AcceptState state );
    static bool         AtTopLevel();
    static bool         Empty() { return mStateStack.empty(); };
        // returns true if there is only one element in the stack

    static void         IgnoreSection() { mfIgnoreSection = true; };
    static void         ReadSection() { mfIgnoreSection = false; };
...
public:
    ...
    // data accesss
    //
    static cErrorBucket*    GetErrorBucket()        { ASSERT( mpError != 0 ); return mpError; };
    static int              GetLineNumber()         { return miLineNum; };
    static void             IncrementLineNumber()   { ++miLineNum; };

    //
    // specify whether to parse entire file for correctness checking
    //
    static void             SetParseOnly( bool f )  { mfParseOnly = f; };
    static bool             ParseOnly()             { return mfParseOnly; };
